...
def featurize(x, anchor, k=7):
    ...
    for arr in x_copy:
        ...
        feature.append(merge_sort_result + merge_hash_result)
...
def bo_loop(dim, benchmark_index, kernel_type):
    ...
    for nruns in range(20):
        ...
        for num_iters in range(n_init, n_evals):
            ...
            torch.save({'inputs_selected':train_x, 'outputs':outputs, 'train_y':train_y}, 'tsp_botorch_'+kernel_type+'_EI_dim_'+str(dim)+'benchmark_index_k7_with_hash_bucket_'+str(benchmark_index)+'_nrun_'+str(nruns)+'.pkl')
