...
def featurize(x, k=5):
    ...
    x_copy = deepcopy(x)
    for arr in x_copy:
        pattern = pattern_count_feature_circular(arr, k)
        feature.append(pattern)
...
def bo_loop(benchmark_index, kernel_type):
    ...
    for nruns in range(20):
        ...
        for num_iters in range(n_init, n_evals):
            ...
            file_name = 'qap_botorch_'+kernel_type+'_EI_benchmark_index_k5_permutation_pattern_'+str(benchmark_index)
